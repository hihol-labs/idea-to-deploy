...
- Теперь, если каждая непустая строка Allowed Change Areas в `.itd/SCOPE_LOCK.md` - один пункт
  списка (`-`, `*`, `+`, `1.` или `1)`, отступ до 12 пробелов или табов, разделители - пробелы
  или табы) ровно с одним путём (можно в `code`) и необязательной пометкой `(new)` или `(updated)`
  в любом регистре, а первая строка Current Task начинается с id активируемого юнита, текст goal
