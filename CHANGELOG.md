...
> `RSI-DEBT-2`, medium risk), PR #284 (юнит `ROUTE-REPAIR-1`, medium risk),
> PR #289 (юнит `ROUTE-REPAIR-2`, medium risk), PR #291 (юнит `RSI-DEBT-3`,
> low risk) и PR #293 с поправкой #294 (юнит `CLOSE-BLOCKERS-1`, medium risk).
> PR #278, #281, #283, #285, #290 и #292 - **ledger-close**, по конвенции репо
> отдельной записи не получают. PR #277 правит только запись приемки юнита
> `REL-1.104.0` (оракул согласованности версии заякорен), а не поставляемое
> поведение. PR #279 и #286 - отчеты ретро: ни скиллов, ни хуков, ни
> контрактов они не меняют и записи тоже не получают.

### Fixed
- **Бесплатная двухфазная квитанция ревью терпит ограниченный обратный шаг
  часов (`BROKER-ISOLATION`).** `tests/verify_review_broker.py` падал примерно
  в 8% прогонов, и на хостовом чекауте тоже: часы хоста шагают назад (два
  шага, -2,478 с и -5,366 с, за 45 секунд замера `datetime.now(utc)`), а
  квитанция требовала строгого порядка отметок. `MAX_CLOCK_SKEW_SECONDS = 60`
  покрывает все четыре сравнения хронологии, которыми владеет квитанция; окно
  свежести не тронуто. Регрессии подставляют обе отметки, поэтому
  детерминированы: до-фиксовые байты красные дважды подряд, 741 -> 835
  проверок. Новый `tests/verify_isolated_candidate_mirror.py` гоняет быстрое
  зеркало внутри материализованного изолированного кандидата и требует его
  последнюю строку байт в байт.
- **Аудит-строка `COMPLETION_BYPASS` больше не попадает в отслеживаемый журнал
  (`RSI-DEBT-2`).** Гейт работает ВНУТРИ `git commit`, поэтому дозапись в
  отслеживаемый `.itd-memory/events.jsonl` делала рабочее дерево отличным от
  отревьюенного кандидата, и exact-tree гейт ревью отказывал тому самому
  коммиту, которому был выдан обход. Путь сидел в поставляемом
  `DEFAULT_POLICY`, то есть задет был каждый проект без своей политики.
  Каноническое место теперь - неотслеживаемый append-only
  `.itd-memory/completion-bypass.jsonl`; оба производителя (`audit_bypass` в
  гейте коммита и `audit_completion_bypass` в шаблоне гигиены) объявляют один
  путь, их расхождение закреплено оракулом. Перенос строк в канонический
  журнал - явный шаг ledger-close (`skills/_shared/itd_bypass_audit.py carry`),
  идемпотентный по id, с отказом, когда один id несет два разных payload.
  Две гарантии структурные: перенос никогда не удаляет леджер, гейт никогда
  не строит цепочки каталогов. Оракул вырос с 30 до 60 проверок, паритет
  hard-gate по хостам остался 101 из 101; live-model evidence перезаписан
  (прогон `20260912T124729Z-b71f64e6`, `tests/verify_live_model_benchmark.py`
  85 из 85).
- **Промпт независимого ревью требует исчерпывающего перечисления находок
  внутри привязанного материала одного вызова (`ROUTE-REPAIR-1`).** Замер
  серии RSI-DEBT-2: десять живых кросс-вендорных раундов подряд на одном
  medium-юните, ноль ложных находок и ни одного чистого PASS; выход ОДНОГО
  вызова - одна-две находки, тогда как привязанный материал держал девять
  различных только за раунды 1-7. Контракт промпта большего не просил:
  `PASSED requires findings=[]` означает, что одной high-находки уже хватает
  для BLOCKED. Исправление - одна константа модуля `ENUMERATION_REQUIREMENT`,
  выдаваемая из единственного доверенного хвоста
  `_trusted_json_output_contract`, поэтому direct-, unit- и integration-промпты
  наследуют ее дословно и после всего недоверенного материала; ни одна схема
  вердикта не меняется. Новый оракул `tests/verify_review_enumeration.py` -
  RED-first, 55 проверок, одиннадцать летальных мутаций. Оракул ОБЪЯВЛЯЕТ свой
  предел: отсутствие смыслового противоречия где-либо в промпте он не
  доказывает. Три ноги efficacy перечеканены вживую на обоих хостах и
  заархивированы как `diagnostic-260950c6`.
- **Маршрут отказывает квитанции, которая ничего не привязывает, а переход
  после мержа получил честный режим (`ROUTE-REPAIR-2`).** На смерженном main
  `candidate_context` в режиме `staged` возвращал `diffHash` пустой строки:
  staged-квитанция после мержа не падала, а успешно привязывала ПУСТОЙ
  кандидат - это хуже отсутствующей квитанции, потому что читается как
  evidence. `assert_candidate_not_empty` отказывает в этом на всех четырех
  местах чеканки и на стороне валидации, но только ПОСЛЕ того, как доказано,
  что квитанция описывает этот кандидат. `itd_goal_verify` принимает явный
  `--candidate-mode`, протянутый через `cmd_verify`, `cmd_reconcile` и
  `recover_goal_transition`: раньше adjudication валидировалась только в
  `staged`, и три юнита подряд (REL-1.104.0, RSI-DEBT-2, ROUTE-REPAIR-1)
  закрывал владелец, а не харнес. Отказы по грязному дереву и merge-родителю
  не изменены. `docs/VERIFICATION_LOOP.md` теперь прямо говорит, что каждое
  звено цепочки ревью несет один и тот же claim id, кто его потребитель и как
  читается отказ. Восемнадцать мутаций, все летальны. Только Windows-нога
  дважды поймала один класс: корень временного каталога в коротком 8.3-виде
  сравнивался на вложенность с длинным путем; оба корня теперь резолвятся.
- **Goal-харнес пишет по одной строке evidence на каждую команду верхнего
  уровня составной `A && B` (`RSI-DEBT-3`).** Составная `verificationCommand`
  исполнялась одним непрозрачным `sh -c`, и в леджер шла только последняя
  строка stdout: запись называла последнюю ногу и не показывала, что первая
  вообще выполнялась. `split_top_level_and` распознает ровно одну форму -
  ноги, соединенные `&&` верхнего уровня, с учетом кавычек и экранирования -
  и ОТКАЗЫВАЕТ на всем остальном (`|`, `||`, `;`, одиночный `&`, скобки,
  обратные кавычки, `<<`, некавыченный `#`, перевод строки, незакрытая
  кавычка, хвостовой обратный слэш), уводя команду в нетронутую прежнюю
  однострочную запись; единственный `&` вне `&&`, который принимается, -
  дублирование дескриптора (`2>&1`). `compound_script` гонит цепочку в ОДНОЙ
  оболочке, так что `A && B` сохраняет состояние после A и замыкается коротко,
  как раньше. На ногу пишется `<command>: exit <rc>, stdout sha256 <16 hex>,
  <last line>`. Нога без статуса после успешного предшественника
  записывается с кодом выхода оболочки, и юнит не верифицируется: прежний код
  в этом случае возвращал ноль предшественника и ВЕРИФИЦИРОВАЛ незаконченную
  цепочку. `tests/verify_goal_tools.py` 83/0, двенадцать летальных мутаций.
- **Явное закрытие сессии перестало быть красным: инвентарь провайдеров ревью
  перепроверен, а стартовый таймаут реально применяется
  (`CLOSE-BLOCKERS-1`).** Оракул свежести независимо проверяет дату инвентаря
  и `observedAt` каждого подписанного двуххостового доказательства;
  доказательства были перечеканены 2026-09-12 и валидны по подписи, протух
  только собственный `checkedAt` записи - 42 дня при `maxAgeDays` 30.
  Инвентарь перепроверен замером (codex-cli 0.146.0 против пина 0.144.3,
  GitHub Copilot CLI 1.0.78 против своего пина, оба официальных источника
  достижимы), дата сдвинута на 2026-09-15, сьют возвращает PASSED с
  `signedDualHostProof` true. Объявленный стартовый таймаут 300 с не
  применялся никогда: когда `.itd/itd_hygiene.py` отсутствует, резолвер
  session-save подменяет пару целиком, исполняется
  `docs/SESSION_EXIT_CONTRACT.json`, а ключа `startupTimeoutSeconds` в нем не
  было - работало умолчание кода 120. Пин в
  `tests/verify_session_hygiene_quality.py` следует за резолвером, а не за
  именем файла: шесть строк резолвера из `skills/session-save/SKILL.md`
  закреплены дословно, оболочка не запускается, а RED-сторона исполняется при
  каждом прогоне (контракт без ключа, 300, 599, строка, bool, float); четыре
  мутации по одной гарантии летальны.

### Changed
- **Видимое пользователю: `startupTimeoutSeconds: 600` в
  `docs/SESSION_EXIT_CONTRACT.json` (`CLOSE-BLOCKERS-1`, PR #293).** Это
  контракт, который исполняется, когда в проекте нет своего
  `.itd/itd_hygiene.py`; раньше ключа в нем не было, и стартовая проба
  закрытия сессии шла с умолчанием кода 120 с. Значение 600, а не 300: быстрая
  регрессия замерена в 259,5 с, стартовая проба - в 281 с, и запас в 19-41 с
  флейкал бы от загрузки машины.
- **Видимое пользователю: литерал статуса строки `anthropic-subscription` в
  `.itd/REVIEW_PROVIDER_FRESHNESS.json` сменен с
  `unavailable-no-paid-subscription` на `unavailable-transport-not-enrolled`
  (`CLOSE-BLOCKERS-1`, PR #293).** Маршрут остается типизированно
  недоступным по причине, которая действительно держится: он не enrolled как
  транспорт ревьюера, подписанного двуххостового доказательства для него нет,
  а кросс-вендорная независимость требует не-Anthropic ревьюера всякий раз,
  когда мейкер - Claude. Литерал захардкожен ровно в одном утверждении
  оракула, и смена сделана RED-first.
- **Текст evidence утверждает только наблюденное (`CLOSE-BLOCKERS-1`,
  PR #294).** `unavailabilityEvidence` той же строки выводил "платная подписка
  Claude теперь есть" из того, что на WSL наблюдался Claude Code 2.1.186.
  Установленный бинарь и его версия состояние подписки не устанавливают, а
  оракул проверяет только литерал статуса. Теперь записана наблюденная
  версия, прежняя причина "нет платной подписки" снята как ненаблюдаемая, а
  не заменена обратным утверждением; статус, `checkedAt` и оракул не
  меняются. Находка независимого кросс-вендорного ревью (gpt-5.6-sol,
  close-r5, medium / evidence-integrity) на смерженном `26794a6`, где более
  ранний заход на байт-идентичном материале прошел чисто.

## [1.104.0] - 2026-09-09

> Цикл покрывает PR #272 (юнит `RSI-DEBT-1`, medium risk) и PR #274 (юнит
> `RSI-ROUTE-P1`, medium risk). PR #273 и #275 - **ledger-close**, по конвенции
> репо отдельной записи не получают.

### Fixed
- **Скраббер больше не редактирует обычный код перед независимым ревьюером
  (`RSI-DEBT-1`).** Детектор секретов срабатывал на идентификаторах, чьё имя
  лишь содержит `token`, и ревьюер получал вместо кода плашку редакции.
  Теперь редактируется значение, а не всякое совпадение по имени; отказ на
  реальном чувствительном материале сохранён, пины точности скраббера
  расширены.
- **Прозрачный маршрут ревью режет по-файловые куски из того же
  скраббленного текста, который отдаёт (`RSI-ROUTE-P1`).** Представление
  возвращало скраббленный `diff_text`, а куски - из до-скрабленных байтов,
  поэтому при единственной редактируемой строке `"".join(chunks) != diff_text`
  и `itd_review_broker._review_units` отвергал весь кандидат с «hierarchical
  review unit coverage is invalid» ещё до вызова модели. Куски теперь режутся
  из возвращаемого текста существующим построителем; равенство закреплено
  запечатанной фикстурой, а мутация «резать из сырых байтов» летальна.

### Boundaries
- Кода по существу сверх PR #272 и #274 релиз не вносит: только версия в
  манифестах, бейджах, пинах документации и release-оракуле, эта запись и
  перепин live-model evidence на релизное дерево.
- Раскатка: релиз - предусловие нативной установки 1.104.0 на WSL и Windows
  и перепина review-authority на выпущенные продюсер и скраббер; до неё
  установленная authority остаётся до-фиксовой 1.103.1.

## [1.103.1] - 2026-09-07

> Патч-релиз покрывает PR #267 (юнит `ROUTE-DEBTS`, high risk). Ledger-close
> PR по конвенции репо отдельной записи не получает.

### Fixed
- **Привязка восстановления маршрута и улик ревью к точному кандидату
  (`ROUTE-DEBTS`).** Проекция улик, типизированная адъюдикация, негативные
  чекпоинты, ограниченное исполнение и нативные операции пространства имен
  ужесточены. Состояние Goal/task восстанавливается из канонических событий
  без дублирования завершения, а проваленные улики сохраняются, а не
  затираются.
- **Точные source/native регрессии и неизменяемая история efficacy.**
  Воспроизведен и исправлен дефект перцентильной фикстуры; исходные байты
  корпуса и его хеш сохранены, скорректированный прогон - отдельное
  наблюдение, а не подмена. Q6 кампания и существующие evidence-gated
  требования публикации и installed-proof сохранены.

### Boundaries
- Кода по существу сверх PR #267 патч не вносит: только версия в манифестах,
  бейджах, пинах документации и release-оракуле плюс эта запись.

## [1.103.0] - 2026-09-04

> Цикл покрывает PR #253, #255, #257, #259. Юниты `PILOT-P01`, `STOPRULE-1`,
> `STOPRULE-2`, `PILOT-P02-CLOSE`. PR #254, #256, #258, #260 — **ledger-close**
> и по конвенции репо отдельной записи не получают.

### Added
- **Закрытая таксономия вердикта ревью (`PILOT-P01`).** `category`, `source` и
  `severity` перестали быть свободным текстом: единый словарь
  `skills/_shared/VERDICT_TAXONOMY.json` (`taxonomyVersion: 1`) и общий
  валидатор `skills/_shared/itd_verdict_taxonomy.py` применяются обоими
  писателями **на записи**. Внесловарная запись отвергается и уходит в карантин
  со счетчиком, а не портит леджер; легаси-записи нормализуются на чтении, так
  что накопленная история остается читаемой. Послабление `unclassified` /
  `unspecified` разрешено через `bySource` только не-ревьюерам и ни при каком
  содержимом словаря не распространяется на `subagent-verdict`.
- **Критерий остановки R7 `SURFACE_TREADMILL` с уликой поверхности
  (`STOPRULE-1`).** Правило видит режим, в котором каждый заход ревью
  наращивает поверхность кандидата, не закрывая класс дефекта, и называет его
  по имени вместо еще одного захода. Улика - измеренный рост поверхности, а не
  суждение агента.
- **Машинерия слепого семантического протокола (`PILOT-P02`).** Замороженный
  контракт замера `skills/_shared/BLIND_PROTOCOL.json` (`protocol_version: 1`)
  и `skills/_shared/itd_blind_protocol.py`: фильтр популяции, воспроизводимая
  жеребьевка без возврата с выведенным из протокола seed, печать с
  tamper-evidence, слепой лист без авторской метки и скоринг с жестким порогом.
  Оракул стоит на **запечатанных фикстурах**, а не на живом леджере, поэтому
  замер перепроверяется на любой машине. Вердикт протокола выносит отдельный
  decision-юнит - здесь только машинерия.

### Changed
- **Стоп-правило 1.2.0: решает само, подтверждает человек (`STOPRULE-2`).**
  Политика переведена в режим `decides-with-human-confirmation`, добавлен
  `--emit-dispositions`: правило само составляет диспозиции ADR-007 по открытым
  находкам, а человек подписывает их одной закрытой фразой. Прежде остановка
  серии и разбор находок целиком лежали на операторе. Маршрут отработал живьем
  на собственной серии правила и на внешнем кандидате: в обоих случаях терминал
  выносило правило, а не оператор, и закрывался он подписанными владельцем
  диспозициями (PR #257, #259). Номера заходов здесь намеренно не приводятся -
  журнал репозитория их не фиксирует, а память сессии уликой не является.
### Boundaries
- Популяция слепого протокола на момент релиза пуста: до раскатки этой версии
  писатели не штампуют `taxonomy_version`, поэтому наблюдение начинается после
  установки, а не в момент мержа.
- **Свежесть провайдера-ревьюера этот релиз НЕ чинит.**
  `verify_reviewer_provider_freshness` остается красным по календарю - он был
  красным и до кандидата. Попытка починить его внутри релизного юнита
  остановлена стоп-правилом (терминал `REDESIGN_OR_DISCARD`): критерий приемки
  дважды утверждал подписанные улики, подлинность которых сам не проверял.
  Корень - схема записи не умеет привязывать улику собственной аттестации;
  работа вынесена в отдельный юнит, долг записан в BACKLOG.

---

## [1.102.0] - 2026-08-31

> Цикл покрывает PR #249. Юнит `REVIEWER-DEFAULT-SOL`.

### Changed
- **Независимый ревьюер по умолчанию - `gpt-5.6-sol`, `gpt-5.6-terra` остается
  запасным (REVIEWER-DEFAULT-SOL).** Брокерская таблица маршрутизации уже
  предписывала anthropic-мейкеру ревьюера `openai-responses` (= Sol), а
  CLI-дефолт продюсера стоял на Terra - два пути маршрута расходились ровно
  на основном сценарии репо. Дефолт `--reviewer-model` приведен к политике.
  Тот же дрейф был зашит вторым местом - в дефолте `--model` рекордера
  efficacy; оба сверяются с головой `routing.claudeMaker`.

### Added
- **Автоматический переход на запасного ревьюера - только транспортный.**
  Подмена модели разрешена исключительно когда вердикта не было вообще
  (`UNAVAILABLE`: auth, запуск, таймаут). Любой полученный вердикт, включая
  `BLOCKED`, закрывает ногу на выдавшей его модели: маршрут не может искать
  более удобного ревьюера. Учет ведет `TransportAttempt`, вынесенный на
  уровень модуля, чтобы правило доказывалось само по себе, а не только через
  живой транспорт.
- **Пометка деградации в квитанции.** Нога, отработавшая на запасной модели,
  несет объявленное поле `degradedFrom`; личность ревьюера остается закрытой
  (посторонние ключи по-прежнему отвергаются), а происхождение обязано
  принадлежать паре Sol/Terra и отличаться от действующей модели. Смена
  модели - смена личности ревьюера, накопленный PASS не наследуется.
- **Оракул сверяет политику с кодом.** `verify_free_reviewer_producer`
  проверяет, что дефолт продюсера равен голове `routing.claudeMaker`, а
  запасной - объявленному альтернату; класс дефектов "политика против кода"
  срабатывал трижды за предыдущий юнит.

### Boundaries
- Обратное направление не изменилось: openai-мейкеру ревьюер по-прежнему
  форсится в противоположную модель, и запасной вариант там не предлагается -
  альтернат совпадает с действующей моделью.
- Неизвестный или смешанный мейкер по-прежнему не имеет автоматического
  маршрута (`unknownMaker: []`, manual blocking review).
- Замер по ходу юнита: Sol НЕ оказался стабильнее Terra на подписке Codex -
  транспорт падал на обеих моделях. Правку это не отменяет (рассинхрон с
  политикой был реален), но ожидание "станет надежнее" не подтверждено.

---

## [1.101.0] - 2026-08-28

> Цикл покрывает PR #226-#244. Содержательные изменения описаны ниже; PR
> #231, #233, #236, #238, #242, #244 — **ledger-close** (перевод юнита в
> `verified` в STATE/леджерах после мержа его кандидата) и по конвенции репо
> отдельной записи не получают: их предмет описан в записи соответствующего
> кандидата.

### Added
- **Правило остановки цикла ревью над СОДЕРЖАНИЕМ находок (LPD-003-3).**
  Раунды останавливали по их числу; замер трёх записанных историй показал, что
  это число не измеряет ничего. Из десяти заходов публикации LPD-003-1
  вердиктов ревьюера было три — остальные семь — отказы предусловия и
  транспорта. В S04b раунд PUB5 дал `PASSED`, а следующие раунды нашли в том
  же механизме ещё четыре реальных дефекта. В R6 тринадцать раундов называли
  каждый раз новый механизм и сошлись на чистом PASS — потолок в три раунда
  отгрузил бы кандидата с воспроизведённым эксплойтом. Сигнал остановки —
  повтор одного ключа механизма (файл, класс дефекта) в двух и более РАЗНЫХ
  раундах: значит дефектна форма решения, а не экземпляр. Новые
  `.itd/STOP_RULE_POLICY.json` (контракт), `scripts/itd_stop_rule.py`
  (классификатор терминалов, ключ механизма, решение, живая проверка привязки
  приёмочной бухгалтерии) и `tests/verify_stop_rule.py` (149 проверок).
  Повтор засчитывается только при доказанной СМЕНЕ КАНДИДАТА между раундами —
  иначе правило говорит `RECURRENCE_UNCONFIRMED` и называет нехватку улики;
  история, где вердиктов нет или последним сорвался маршрут, получает
  `ROUTE_REPAIR`, а не приглашение на новый раунд.
  Правило **advisory**: печатает терминал и основание, решение принимает
  владелец; гейтом не становится, и это заморожено в политике.
  Проверка — реплей записанных историй с байт-сверкой улик по sha256
  (`tests/references/stop-rule/`); раунды, чьи отчёты вырезаны prune-политикой,
  помечены классом `narrative` и машинной уликой не считаются.
  Врезка — `docs/VERIFICATION_LOOP.md`.

- **Targeted-профиль регрессии: полный прогон перестал быть входом по умолчанию
  (LPD-003-1).** По замеру GATE G0 `tests/run-all.sh` — 859 из 1054 минут
  машинного слоя (82%). `bash tests/run-all.sh --targeted` берёт набор из
  `.itd/IMPACT_GRAPH.json` через новый `scripts/itd_regression_select.py`
  (тот же аудированный источник, что и профили верификации), а пути, которые
  нельзя перечислить узлами — контракты юнитов, записи live-прогонов,
  квитанции — разрешаются правилами нового `.itd/IMPACT_PATTERNS.json`. Каждое
  правило `no-impact` судится машинно против сгенерированной карты, поэтому
  «влияния нет» — проверяемое утверждение, а не декларация. Границы гарантии
  печатаются: `OUTSIDE-MIRROR` (сьют связан картой, но зеркало его не гоняет —
  назван, не прогнан), `NO-IMPACT by rule`, `MIRROR-COVERAGE` (129 из 154
  сьютов) — чтобы `DONE fails:none` не читалось шире, чем есть. Замер на этом
  репозитории: полный прогон 301-323 с; targeted 99 с на однофайловой правке
  (−67%), 210/223/202 с на трёх реальных юнитах (−30/−26/−33%) и 218 с на
  собственном кандидате этого юнита (48 сьютов, −33%). Новый оракул
  `tests/verify_targeted_regression.py` (98 проверок, включая мутации карты,
  заведомо ложное правило, живую git-песочницу для среза правки и
  поведенческую сверку ЧИСЛА в строке покрытия для quick- и полного профилей).
  **Периметр селектора сужен** (решение владельца 2026-08-24,
  `.itd/DECISIONS.md`): ни один публичный вход не называет читаемый им файл —
  карта, правила и зеркало берутся из ЭТОГО репозитория по расположению самого
  скрипта, флаг `--graph` удалён вслед за `--root`/`--rules`. Мутационные тесты
  ходят внутренним швом `load_graph()`/`select()`. Целостность карты усилена:
  неканоническое ребро или узел (`tests/x/../verify_*.py`, абсолютный путь,
  обратный слэш) уводит в strict, а не сужает прогон по битой карте. В
  `run-all.sh` починен `--`: путь, начинающийся с дефисов, больше не
  возвращается во внешний разбор опций как «unknown flag».
  Срез правки без `--changed` — объединение четырёх проб (коммиты сверх базы,
  staged, unstaged, untracked) с источником каждого пути в `changedSources`:
  приоритетная проба молча теряла новый staged-файл на ветке с коммитами, а
  `git diff` не сообщает про untracked вовсе. Упавшая проба — неизвестное
  влияние (strict), а не пустой срез. Набор изменённых
  путей, целиком объяснённый правилами `no-impact`, уходит в полный прогон —
  зелёное без единого прогнанного сьюта не является уликой. Селектор судит
  ТОЛЬКО то дерево, в котором лежит сам: override-флагов `--root`/`--rules`
  нет по решению владельца — они не использовались ни одним вызывающим, а
  вокруг них накопился класс дефектов про чужие и поддельные деревья,
  недостижимый с автоматического маршрута. Нечитаемое зеркало fail-closed
  (пустое множество делало `outsideMirror` вакуумным), имена сьютов в
  правилах сверяются с файлами `tests/<name>.py` и обязаны входить в зеркало —
  иначе правило обещало бы покрытие, которого targeted не даст; каждое правило
  дополнительно проверяется ПРОГОНОМ селектора на совпадающем пути, а не
  только существованием файлов. Шаблон правила не переходит
  границу каталога (`*` в `fnmatch` означал «в том числе через `/`», и правило
  про один каталог говорило за всё поддерево), а пути `--changed` передаются
  позиционными аргументами: путь с пробелом раньше разъезжался на два.
- **`bash tests/run-all.sh --fail-fast`** — красный прогон больше не
  доигрывается до конца (128 из 859 минут истории приходились на прогоны,
  уже ставшие красными). Останавливается на первом красном сьюте, называет
  его и остаётся ненулевым.

### Fixed
- **Отсутствующий host-owned вход больше не выглядит красным сьютом
  (LPD-003-1).** `.itd-memory/` git-ignored, поэтому в изолированном дереве
  машинного продюсера пина эффективности нет по построению — раньше это
  печаталось как `FAIL verify_independent_review_efficacy` и было неотличимо
  от сломанного кандидата (34 таких прогона в истории квитанций). Теперь это
  отдельный класс `BLOCKED` с WHY и точной FIX-командой
  (`itd_verification_loop.py machine --input <pin>`); код выхода остаётся
  ненулевым — молчаливый пропуск был бы false-green.
- **checker называет все нарушения pre-flight за один прогон (GENG-S04B).**
  report/prompt/phase-one/keyring/route-evidence валидируются пакетно; при
  нескольких нарушениях — один `LoopError` со всеми WHY+FIX, при одном —
  прежний точный текст. Замерено: три холостых прогона подряд на GENG-S03.
  По находке независимого ревьюера (PUB1, medium): невалидный путь артефакта
  больше НЕ скрывает нарушения самого маршрута — структурная валидация
  phase-one/keyring/кандидата выполняется и без привязки к байтам отчёта
  (reporting-only режим `bind_artifacts=False`, за которым всегда следует
  отказ, поэтому квитанция из частично проверенного маршрута невозможна).
  По находкам PUB2 (medium x2) агрегация доведена до независимости КАЖДОГО
  входа: phase-one и keyring резолвятся по отдельности (две плохие ссылки —
  один прогон), а разбор отчёта перенесён внутрь pre-flight (сломанный отчёт
  называется вместе с другими нарушениями, а не на следующем прогоне).
  По находке PUB6 (medium) каждая route-ссылка проверяется на форму отдельно
  (сломанная ссылка не прячет некорректного соседа), а гарантия объявлена
  честно прямо в тексте ошибки: в прогоне называются все НЕЗАВИСИМО
  проверяемые входы, а проверки, требующие валидности другого входа
  (верификация подписи, привязка артефактов), выполняются после починки
  предпосылки.

- **Живая улика бенчмарка перестала инвалидироваться правкой, которой она не
  касается (LPD-003-2).** Хеш инвалидации считался по десяти корням целиком
  (~231 файл), а исполняет прогон ~10% из них — любая правка в `skills/`,
  `hooks/` или `agents/` роняла Gate 1 и требовала живого перегона (за одну
  сессию 2026-08-12 поймано шесть раз). Пин сужен до ИЗМЕРЕННОЙ исполняемой
  поверхности (60 файлов), объявленной один раз в `tests/itd_benchmark_pin.py`
  и импортируемой и раннером, и оракулом: статическая половина равна
  задекларированной `STATIC_EXECUTABLE_SURFACE`, а каждый динамический корень
  обязан покрывать хотя бы одно структурно измеренное чтение из транскриптов
  (52 записанных прогона, только события исполнения команд). Сужение доказано
  мутациями в обе стороны, а не заявлено. Гард `cd`-целей fail-closed к
  написанию команды: кавычки, `cd --`, `pushd` и любая НЕРАСПОЗНАННАЯ команда
  со сменой каталога и отдельно стоящим путём `.itd-plugin` консервативно
  считаются входом в корень плагина — измерение расширяется, никогда не
  сужается. Грязные промежуточные прогоны из записи выброшены: улика
  перечеканена на чистом дереве.

- **Два предсуществующих красных оракула закрыты по корню, класс сьютов вне
  зеркала назван машиночитаемо (ORACLE-DEBT).** `verify_operating_loops_release`
  оказался не протухшим пином, а релизным оракулом, зелёным ПО ПОСТРОЕНИЮ
  только на своём кандидате (машинно: exit 0 на `172fca79`, exit 1 на main) —
  файл сьюта не тронут, класс объявлен в реестре как candidate-bound с
  исполнимой командой. В `verify_harness_demo_absorption` недостижимые
  git-объекты заменены маршрутом через запечатанную фикстуру с теми же
  base/accepted trees, `changedPaths` и печатями блобов; git-дельта осталась
  опциональным усилением, контракт поднят до v5, гардов мутаций 34 -> 38.
  Все 26 сьютов вне зеркала измерены (rc, длительность, причина); 16 зелёных
  возвращены — `MIRROR-COVERAGE` 124/150 -> **140/150**, оставшиеся 9 объявлены
  в новом `tests/OUT_OF_MIRROR.json`, а `verify_runall_drift.py` краснеет на
  неклассифицированном сьюте, на плейсхолдере вместо исполнимой команды и на
  неконкретной фазе.
...
- **Пять пар сьютов с доказанно идентичным покрытием слиты (LPD-003-4).**
  Слияния выбраны не на глаз, а по сгенерированной impact-карте, обе стороны
  внутри зеркала: `signal_attribution<-completion_ledger`,
  ...
  Сьютов на диске 155 -> 150, зеркало 129 -> 124; набор узлов карты на шаге
  слияния сохранён побайтово, а живучесть проверок донора в хранителе доказана
  живыми мутациями предмета с байт-точным восстановлением. Дрейф продолжает
  ловиться: инвариант `consolidation_invariants` в `verify_runall_drift.py`.
  Шестая пара (`brownfield_and_gate` + `skill_enforcement`) слита НЕ была —
  мутация показала, что хранитель остаётся зелёным на дефекте, который ловит
  донор.

- **Перемер машинного слоя после targeted-профиля; NO-GO по GENG-B
  подтверждён (accounting only).** Тот же разрез G0, эры разделены по мержу
  LPD-003-1: в post-эре `run-all` занимает **0% машинного слоя** (было 82%),
  потолок кэша — 0 из 8 юнитов берут порог 30 мин (медиана 1.42, максимум
  27.9), остаточный адресуемый пул ~2 ч/мес против 5.8 ч/мес в поправке.
  Предусловие поправки выполнено: targeted-маршрут забрал минуты дешевле, чем
  их забрал бы кэш. Вердикт владельца 2026-08-27 — GENG-B NO-GO подтверждён,
  дата ревью 2026-09-28 сохранена.

- **Программа GENG закрыта целиком измеренным stop-вердиктом (GENG-C-EXP).**
  Единственный уцелевший пункт исполнен по пре-регистрированному правилу.
  Фаза 0 (ретроспектива серии по байтам промптов): 12 PRE / 1 NEW / 2 BASELINE,
  доля PRE среди классифицируемых 0.923. Фаза 1 (12 замороженных кандидатов x
  4 независимых ревьюера): по валидной популяции 11/12 доля ложных PASSED
  0.143 у одиночного ревьюера и 0.143 у кворума 2-из-3, перекрытие промахов
  1.0, шум на чистых 0/16 — заранее объявленный порог (кворум <= 0.5 x
  одиночный) **NOT MET**. Выигрыш только в латентности: 172 с против 430 с.
  Кандидат C12 исключён из метрик по находке независимого чекера (эксцерпт был
  обрезан до дефектного блока, PASSED были правильными вердиктами) —
  исключение объявлено, байты не менялись. Вердикт владельца: stop; opt-in не
  вводится, GENG-код в маршрут не добавляется. Замер заморожен fail-closed
  чекером (50/0 полный, 27/0 изолят-разрез), опровергнут мутацией.

- **Гигиена леджеров (accounting only).** Пять протухших строк
  quality-леджера перечеканены живыми прогонами улик (все 14 оракулов exit 0)
  под строгий session-exit контракт «свежее 30 дней»; в `BACKLOG.md` записан
  измеренный дефект иерархического разреза диффа (ложный `NameError` на
  публикации LPD-003-4) как отдельный кандидат, а не как «шум ревьюера».
...
- **The LPD-002 debt session (A1-A9) closes the plan's own route debts**
  ...
  - **A2, completion gate stale evaluator** (`hooks/completion_lib.py`,
    oracle 15 -> 83): display/write commands are never runtime signals
...
- **Proportional suites from a declared impact map (LPD-002 R6, the last point
  ...
  - *Both mutation sides* in `tests/verify_verification_profiles.py`
    (57 → 109 checks): drop every edge to a suite → completeness fails; drop an
    ...
    any characters — a charset regex silently dropped dotted names), invalid
    glob grammar fails closed instead of leaking pathlib's ValueError, and
    --check compares the whole map document so generator-owned fields cannot
    drift silently, and both map sections must be present explicitly — a
    missing generated/declared section fails closed instead of defaulting to
    an empty graph.
...
### Fixed
- Native Windows `itd gate doctor` now gives committed-head validation on UNC
  checkouts a bounded 180-second cold-start budget while retaining the strict
  30-second default for local paths. Receipt/path binding, stale evidence and
  all fail-closed verdicts are unchanged.
