...
- Пилот PILOT-LOW-1 показал: strict-матчер `skills/_shared/itd_risk_classes.py` читал текст
  goal безусловно, поэтому один и тот же модуль получал low по docstring и high по подробному
  описанию со словом "payment", а юнит, меняющий только тесты, не мог уйти с маршрута high.
- Теперь, если каждая непустая строка Allowed Change Areas в `.itd/SCOPE_LOCK.md` - один пункт
  списка (`-`, `*`, `+`, `1.` или `1)`, отступ до 12 ASCII-пробелов) ровно с одним путём (можно в
  `code`) и необязательной пометкой `(new)` или `(updated)` в любом регистре, а первая строка Current Task начинается с id активируемого юнита, текст goal
  (ключевые слова и пути) не источник strict-классов. Тестовый путь - каталог `tests/`, `test/`,
  `__tests__/` или файл кода `test_*.py`, `*_test.*`, `*.test.*`, `*.spec.*`, `conftest.py`,
  записанный только ASCII-символами `[A-Za-z0-9_./*?-]` без сегмента `..`; набор символов
  проверяется по токену как есть (только `\` заменяется на `/`), пробелы не обрезаются,
  регистр приводится уже после проверки. `itd_unit_log.py activate` печатает
...
- Пол сохраняется: путь или ключевое слово strict-класса в самой области и любая строка вне
  ...
  Current Task, который не начинается с id юнита (устаревшая область прошлого юнита, даже
  упоминающая новый юнит в другом месте), оставляют goal источником и поднимают тир до high;
  для последнего случая `activate` печатает подсказку. Проекция STATE харнеса цели сбрасывает
  `riskTierExempt`. Грамматика - белый список по решению владельца после того, как стоп-правило
  остановило серию ревью лексера с чёрным списком (REDESIGN_OR_DISCARD на раунде c2).
- Шаблон `docs/templates/itd/SCOPE_LOCK.md` просит начинать Current Task с id юнита; поправка в
...
- Новый оракул `tests/verify_tier_source.py` (в `tests/run-all.sh`): шесть синтетических пар в
  стиле пилота дают один тир на tests-only области, пол держится в 24 случаях; на дофиксовых
  байтах 39 проверок красные, `--mutations` - 16 из 16 летальны.

### Changed - G-004 HOOKS-AUTOINSTALL-1: /adopt и /project предлагают установить enforcement-хуки

- Новый stdlib-хелпер `skills/_shared/itd_project_hooks.py`: `plan --root <проект>` -
  read-only список того, что добавится в `<проект>/.claude/settings.json`;
  `apply --root <проект> --yes` - идемпотентное слияние шаблона
  `skills/adopt/references/project-settings-template.json` (хук = событие + matcher +
  команда, поэтому `state-guard.sh` регистрируется под каждым своим matcher'ом; правила
  `permissions.ask` - по точной строке). Без `--yes` apply отказывает; без аргументов -
  no-op. Чужие хуки, ключи и `permissions` сохраняются; повторный apply не меняет байты и
  mtime; хук, скрипт которого уже зарегистрирован в user-level `~/.claude/settings.json`
  из каталога хуков idea-to-deploy (каталог `hooks` рядом с этим файлом или каталог хуков
  плагина), на уровне проекта не дублируется (остальные хуки и `permissions` ставятся;
  одноимённый скрипт из чужого каталога, в том числе `.claude/hooks` другого проекта, не в
  счёт; каталог узнаётся и в формах `~`, `$HOME`, `${HOME}`, с `..` и через симлинк); user-level файл не
  пишется никогда; симлинк на settings обновляется через цель внутри проекта, и `plan`
  называет реальный путь; путь, который (через симлинк на `settings.json` или на `.claude`,
  живой или висячий) уходит за пределы `--root`, симлинк-петля (на `settings.json` или на
  `.claude`), битый JSON, NaN/Infinity (в том числе `1e999`), не обычный файл вместо
  `settings.json`, записи хуков неверной формы (строгая схема без умолчаний), шаблон неверной
  формы или пустой, `--root` с петлёй симлинков или несуществующий - ненулевой exit с WHY/FIX
  без записи; любой непредвиденный сбой (глубокая вложенность, `.claude`-файл или без прав,
  длинный путь) - тоже отказ с WHY/FIX без трейсбека, и отказ честно говорит, была ли уже
  запись; нечитаемый user-level файл или каталог - видимое предупреждение в `plan`.
- `/adopt`: Step 2 больше не пишет `.claude/settings.json` вручную; новый финальный
  Step 7 показывает план хелпера, спрашивает подтверждение и запускает `apply --yes`
  только после «да» (до передачи в `/strategy`/`/blueprint`; `skip-chain` его не
  пропускает). Поведение изменено:
  раньше хуки ставились без вопроса, теперь при отказе проект остаётся без них.
- `/project`: новый финальный Step 6 с тем же предложением для созданного проекта.
- `README.md`/`README.ru.md`: вместо «not auto-installed» - описание шага установки.
- Оракул `tests/verify_hooks_autoinstall.py` (в run-all): 149 проверок (матрица неправильных форм настроек и шаблона x
  `plan`/`apply`, состояния `.claude`, `settings.json` и `--root`), RED на дофиксовых байтах,
  мутации 28/28 с контрольной копией.
- `tests/fixtures/fixture-17-adopt/notes.md`, `scripts/itd_install_runtime.py`
  (`RUNTIME_SHARED_FILES`) - синхронизированы с новым шагом и хелпером.
...
> low risk), PR #293 с поправкой #294 (юнит `CLOSE-BLOCKERS-1`, medium risk),
> PR #297 (юнит `LEDGER-ARCHIVE-1`, low risk) и PR #299 (юнит `ROUTE-REPAIR-3`,
> low risk). PR #278, #281, #283, #285, #290, #292, #296, #298 и #301 -
> **ledger-close**, по конвенции репо отдельной записи не получают. PR #277
> правит только запись приемки юнита `REL-1.104.0` (оракул согласованности
> версии заякорен), а не поставляемое поведение. PR #300 (юнит
> `ROUTE-DEBTS-ORACLE-1`, low risk) правит только оракул
> `tests/verify_route_debts.py` (подставные `validate_adjudication` принимают
> тот вызов, который делает production) - поставляемое поведение не меняется,
> записи он не получает. PR #279 и #286 - отчеты ретро: ни скиллов, ни хуков,
> ни контрактов они не меняют и записи тоже не получают.

### Added
- **Метрика маршрута различает три вида `verified` (`ROUTE-REPAIR-3`, low
  risk).** `lifecyclesVerified` / `unitsVerified` считали любой verified-цикл
  одним и тем же: на живом логе 17 переходов харнеса без квитанции чекера и 3
  записи рукой владельца были неотличимы от полной независимой верификации.
  `itd_unit_lifecycle.build` теперь относит каждый verified-цикл ровно к одному
  классу - `verifiedOwnerRoute` (метка маршрута называет владельца или писатель
  не харнес), `verifiedIndependent` (переход харнеса с привязанной квитанцией
  чекера), `verifiedMachineOnly` (переход харнеса без неё) - и отдаёт поле
  цикла `routeClass`; `scripts/itd_metrics.py` выводит все три счётчика, их
  сумма равна `unitsVerified`. Owner-маршрут побеждает привязанную квитанцию:
  путь, вписанный рукой рядом с переходом, записанным рукой, никто не
  валидировал. Написание нормализуется, а не принимается на веру: `owner`,
  `owner-route`, `Owner_Route` - одна метка, литерал `none` / `null` в
  `checkerReceipt` - отсутствие квитанции. `vcr` и его определение не
  изменились (фикстура 0.933 и живой каталог 1.0 до и после правки). Живой
  каталог на дереве `dba30dd`: 0 / 17 / 3 при `lifecyclesVerified` 20. Новый оракул
  `tests/verify_route_metric.py` (58 проверок, в зеркале): RED-first 38 красных
  до правки; 6 мутаций исполняются внутри оракула на копии модуля, каждая
  обязана сменить класс названных юнитов, не тронуть остальные и не сдвинуть
  `vcr`; мутация с пропавшим якорем красная, а не пустая. Известный предел:
  харнес пока не пишет `checkerReceipt` в verified-событие, поэтому
  `verifiedIndependent` для его переходов остаётся 0 до отдельного юнита.

### Fixed
- **Атрибуция событий переживает архивирование леджера (`LEDGER-ARCHIVE-1`,
  low risk).** `itd_unit_lifecycle.attribute` безусловно доверял явной метке
  `ledger` на строке события. Архивирование переименовывает `GOAL.json`, имя
  занимает новый леджер, и метка начинает называть файл, который юнитом не
  владеет: замер 2026-09-20 - 40 строк и 13 циклов PE5-эры числились за новым
  `GOAL.json`. Теперь метка принимается, только пока она называет владельца
  юнита или псевдо-леджер `STATE`; иначе при ровно одном владельце событие
  уходит к нему с причиной `explicit-stale-sole-owner`, а во всех остальных
  случаях считается неатрибутированным (`explicit-not-owner`) - окно дат к
  ложной метке не применяется. На живом каталоге 13 циклов вернулись к
  `GOAL-2026-09-20.json`; `lifecyclesVerified` 18, `unattributedEvents` 0 и
  `vcr` 1.0 не изменились. RED-first: 6 новых проверок были красными до правки,
  8 мутаций убиты. Заодно закрыты два известных красных места того же оракула
  (`tests/verify_ledger_reconciliation.py`, красный на `main` с 2026-09-03):
  проверка порядка «событие раньше STATE» резала ветку по первому `return 0` и
  после раннего выхода восстановления (#267) не находила ни одного вызова
  (`@-1`) - теперь сегмент равен всей ветке, мутация порядка убита для
  `verified` и `close`; проверка «в живом логе >=1 blocked» пинила факт
  ротируемого gitignored-лога и потеряла предмет - заменена инвариантом «ни
  один цикл не лежит под леджером, не владеющим его юнитом» (гарантия про
  blocked остаётся на замороженной фикстуре, секция D). Оракул: 80/3 -> 91/0. По находкам независимого ревьюера
  (2026-09-21) оракул дополнен до 96/0: неоднозначный владелец и отсутствие
  владельца сверяются вместе с причиной `explicit-not-owner`, нестроковая метка
  `ledger` не считается явным заявлением, сегмент ветки `backfill` закрыт
  точкой входа модуля; каждая из трёх правок подтверждена мутацией.
...
- Раскатка: релиз - предусловие нативной установки 1.105.0 на WSL и Windows и
  перепина review-authority на выпущенные модули `skills/_shared` (учёт
  маршрутов `ROUTE-REPAIR-3`, атрибуция `LEDGER-ARCHIVE-1`); до неё
  установленная authority остаётся 1.104.0.
