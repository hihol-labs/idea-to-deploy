...
**N8 / GENG-C-EXP (2026-08-28) — исполнен и закрыт вердиктом stop:** кворум
одинаковых моделей не снимает ложных PASSED (0.143 -> 0.143 по валидной популяции
11/12; C12 исключён как invalid excerpt; перекрытие 1.0), выигрыш только в
латентности (172 с против 430 с кворумной руки). Пре-регистрация,
замер и fail-closed чекер: `.itd-memory/measurements/GENG-C-EXP-*` +
`benchmarks/geng/`. Программа GENG закрыта целиком; opt-in не вводится.
...
- [x] **LPD-003-1 — `run-all` как машинный оракул -> fail-fast +
...
- [x] **LPD-003-2 — сузить `METHODOLOGY_TREE_ROOTS` до фактически влияющих
...
- [x] **LPD-003-3 — правила остановки в формулировке «находки в САМОЙ правке
...
- [x] **LPD-003-4 — консолидация сьютов по impact-карте.** 151 сьют, 341 узел,
  ...
  Закрыто PR #237/#238/#239 (2026-08-27): 155 -> 150 сьютов, зеркало 124.

**N6 / LPD-003-REMEASURE (2026-08-27) — перемер на разрезе G0 выполнен.**
Post-эра (8 юнитов после merge LPD-003-1): run-all в машинных ногах 0.0 мин
(0% против 82% на G0); потолок кэша 0/8 юнитов >= 30 мин, оба разреза ключа
совпали. **Вердикт владельца: NO-GO по GENG-B подтверждён** — предусловие
адвокатского амендмента выполнено, targeted-маршрут забрал минуты дешевле
кэша. Запись: `.itd/DECISIONS.md` 2026-08-27; числа заморожены коммитнутым
`.itd-memory/measurements/LPD003-REMEASURE-n6.json` + fail-closed сверка
`LPD003-REMEASURE-check.py`; зеркало вне репо `~/.claude/geng/N6/`.
Остаток плана: N7 ORACLE-DEBT -> N8 GENG-C-EXP -> N9 RELEASE.

**N7 / ORACLE-DEBT (2026-08-28) — оба долговых оракула разобраны, класс сьютов
вне зеркала закрыт решением владельца.** `verify_operating_loops_release`
переклассифицирован (зелёный на своём кандидате `172fca79`, красный на любом
другом по построению), `verify_harness_demo_absorption --phase contract`
починен контрактом v5 (маршрут исторического ремонта — запечатанная фикстура
вместо недостижимых git-деревьев; гарантии те же, гардов 34 -> 38). Класс вне
зеркала: 16 измеренно зелёных возвращены (строгий счёт 125/25 -> 141/9),
оставшиеся 9 объявлены в `tests/OUT_OF_MIRROR.json` под fail-closed оракулом.
Слияние пары `brownfield_and_gate`/`skill_enforcement` опровергнуто мутационной
уликой — идентичность покрытия по карте не равна идентичности по поведению.
Побочно закрыт третий, календарный красный (`verify_harness_docs_freshness`).
Запись: `.itd/DECISIONS.md` 2026-08-28; замер —
