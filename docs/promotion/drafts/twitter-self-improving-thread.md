...
We built a Claude Code plugin with 25 skills. Then we built a system that audits itself -- and it keeps finding real drift we missed.

Here's how the self-improvement loop works. Thread.

## Tweet 2

The plugin: idea-to-deploy. 25 skills + 7 subagents for Claude Code.

Full lifecycle: idea -> architecture -> code -> tests -> review -> deploy.

But the hard part isn't building skills. It's keeping 25 skills, 7 agents, READMEs, badges, and tables in sync.
