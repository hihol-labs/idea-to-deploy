...
| F-06 | Mandatory provider-neutral keyless independent checker (OpenAI → Anthropic → GitHub Copilot) | adopt | `docs/adr/ADR-004-mandatory-keyless-review-route.md` | Typed `UNAVAILABLE` after ordered exhaustion; required acceptance remains `UNVERIFIED` until Verification Loop gets eligible evidence |
...
| F-18 | Graduated compaction / context budgeting (full, K4) | adopt (partial, v1.71.0 — transcript-size датчик 60%→/handoff; full budgeting остаётся defer) | `hooks/cost-tracker.sh` (context sensor, retro 2026-07-09 P2; боевой сигнал — amnesia-lab: единственная long-session стратегия остановлена лимитом харнеса на полпути) | текстовое правило 60% в `skills/handoff/SKILL.md` + global-claude-md (v1.70.0, эвристика без датчика) + `hooks/context-budget.sh` + `/session-save` nudge |
...
- **G-003 fixture-proof** (`tests/verify_feature_ledger_fallbacks.py`):
  ...
  здесь fallback (минимум F-05 worktree→freeze, F-06 external reviewer→typed
  `UNAVAILABLE` without fabricated findings).
