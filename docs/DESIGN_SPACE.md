...
| `context-aware.sh` | Stdin (payload `cwd`); с G-003 чтение `hooks/TIER_EXEMPT.json` и `.itd-memory/STATE.json` / `GOAL.json` проекта из `cwd` - на активном low-юните ранний выход без записи | `/tmp/claude-context-{session_id}.json` (counter state) | Нет |
