...
### Исполняемый targeted-прогон регрессии (LPD-003-1)

Селектор профилей отвечает, ЧТО проверять; `scripts/itd_regression_select.py`
отвечает, КАКИЕ сьюты это в терминах `tests/`, и `bash tests/run-all.sh
--targeted` их гоняет. Источник один и тот же — `.itd/IMPACT_GRAPH.json`,
уже судимый операцией `impact-audit`, поэтому второго мнения о влиянии в
системе не появляется.

Динамические пути (контракты юнитов `.itd-memory/contracts/*.md`, записи
live-прогонов, квитанции) узлами карты не станут никогда — их разрешают
правила `.itd/IMPACT_PATTERNS.json`. Правило `effect: "suites"` называет
сьюты; правило `effect: "no-impact"` действительно ровно до тех пор, пока
генератор не нашёл НИ ОДНОГО узла такой формы — это проверяется машинно
(`tests/verify_targeted_regression.py`), включая мутацию заведомо ложным
правилом. Без правил targeted не применялся бы вообще: на замере 20 последних
коммитов два юнита из трёх уходили в strict из-за одного леджерного файла.

Коды выхода селектора: 0 — выбор напечатан, 3 — strict (полный прогон),
2 — ошибка входа. Strict — это не сбой, а честный отказ сужать.
