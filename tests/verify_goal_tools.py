...
def candidate_mode_checks() -> None:
    ...
    with tempfile.TemporaryDirectory() as td:
        # resolve(): on Windows mkdtemp hands back the 8.3 short form
        # (C:\Users\RUNNER~1\...) while the Verification Loop prints the long
        # one (C:\Users\runneradmin\...), and relative_to then raises. Only
        # the Windows CI leg catches this - every local Linux run is green.
        ...
        rel_adj = adj.resolve().relative_to(root).as_posix()
