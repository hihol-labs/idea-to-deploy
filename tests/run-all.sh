...
run_py() {
  ...
  case "$t" in
    verify_independent_review_efficacy)
      ...
      if [ ! -f "$host_pin" ]; then
        echo "FAIL $t (host-owned efficacy keyring pin is not provisioned)"
        fails="$fails $t"
        return 0
      fi
...
verify_otel_export \
verify_platform_tmp_and_new_hooks verify_goal_tools verify_ledger_reconciliation verify_goal_bounded_autonomy verify_goal_five_star verify_retro_scan verify_completion_adversarial_corpus \
...
verify_project_checks verify_review_import verify_work_deadline_benchmark verify_authority_check"
