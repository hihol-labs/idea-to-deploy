...
These 28 hooks turn the methodology from "use it if you remember" into "you literally cannot forget". Without them, even Claude itself will skip the methodology under time pressure (verified the hard way during a 2026-04-07 production incident — see [the case study](#case-study) below).
...
| `state-guard.sh` *(v1.75.0)* | After every Write/Edit/MultiEdit/NotebookEdit (PostToolUse) | ACID-audit fix #4, two duties: (1) when the edited file is a state ledger (`.itd-memory/STATE.json` / `GOAL*.json`), it re-validates the file IMMEDIATELY via `validate_state_core.py` (same invariants as `scripts/validate_state.py`: fail-closed fields, WIP=1 across both ledgers, reconciliation with `events.jsonl`) and returns violations to the model as a red mark (`additionalContext`, FAILED/WHY/FIX) — consistency moves from "checked on next session boot" to "checked after each mutation"; (2) heartbeats `.active-session.lock` on every Write/Edit so the parallel-session warning no longer goes stale between `/session-save` calls — a FRESH lock owned by ANOTHER session is never overwritten (that would mask exactly the parallel session the warning exists for). Disable: `ITD_STATE_GUARD=0`. | No — soft additionalContext only (PostToolUse cannot veto) |
...
All 28 hooks are written in Python 3 (works out of the box on macOS/Linux/WSL), depend only on the standard library, and exit silently in degenerate cases (bad JSON, empty payload, not in the methodology repo) — they never break your session on unrelated work.
