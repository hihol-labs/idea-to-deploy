...
| `crash-recovery.sh` *(v1.18.0)* | After Write/Edit/`git commit` Bash (PostToolUse) | Auto-saves a lightweight checkpoint (last N tool calls, branch + last commit, timestamp, detected session goal) to `claude-checkpoint-<session>.json` every N significant calls, so the next session can reconstruct work after a crash. Opt-in via settings.json. | No — pure side-effect telemetry |
