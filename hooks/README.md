...
| `model-policy.sh` *(v1.83.0)* | Before Task/Agent spawn (PreToolUse, matcher `Task\|Agent`) | Advisory model-policy hint: when a subagent spawn OVERRIDES the agent's frontmatter `model:` with a WEAKER tier (haiku < sonnet < opus), injects an additionalContext reminder of the risk-tier ⇒ model monotonicity invariant (G-003, `tests/verify_model_risk_monotonic.py`). No override / unknown tier / unknown agent → silent. Real per-agent token attribution lands in the cost ledger (`by_agent`). | No — advisory only |
