...
| `cross-review-precommit.sh` *(GPG-003)* | Before every Bash command matching `git commit` (PreToolUse) | Always reminds on sensitive staged paths that `/review` and `/cross-review` converge on the mandatory `OpenAI -> Anthropic -> Gemini` keyless producer. It never egresses code or writes evidence; the exact-candidate Verification Loop gate remains authoritative. Caller flags do not suppress the reminder. | **No — the reminder never blocks or grants acceptance.** |
