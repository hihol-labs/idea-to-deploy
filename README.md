...
| `/cross-review` | Mandatory independent pre-PR review over the exact staged candidate. Uses one fresh opposite-GPT `Sol -> Terra` / `Terra -> Sol` route through `itd_free_reviewer_producer.py`, with no automatic provider fallback or quorum. There is no caller bypass, and acceptance remains owned by evidence-first Verification Loop adjudication. Anthropic, Copilot, Antigravity and paid adapters are optional operations infrastructure. |
