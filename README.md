...
> This repository is a **Claude Code plugin** (see `.claude-plugin/plugin.json`). Installing it registers 35 skills and 10 subagents into your Claude Code environment — it does not run as a standalone CLI.
...
**idea-to-deploy** is a methodology, not just a set of tools. 35 skills + 10 specialized agents that turn Claude Code into a professional developer with a proven pipeline:
...
2. **Propose a new skill** — skills live under `skills/<name>/SKILL.md` and follow the shape documented in the existing 35. Each needs: frontmatter (name, description, triggers, allowed-tools, recommended model), Instructions, Examples, Troubleshooting.
