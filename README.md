...
| `/cross-review` | Mandatory independent pre-PR review over the exact staged candidate. Uses one keyless `OpenAI -> Anthropic -> Gemini` route through `itd_free_reviewer_producer.py`; only `UNAVAILABLE` advances, there is no caller bypass, and acceptance remains owned by Verification Loop adjudication. The separately configured paid adapter is optional operations infrastructure, not a fallback. |
