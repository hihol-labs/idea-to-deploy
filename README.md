...
The methodology is only effective if Claude actually invokes the skills. Trigger word matching in `description` is necessary but not sufficient — under time pressure or with ambiguous prompts, Claude may default to ad-hoc tool calls. The `hooks/` folder contains **21 hooks** that close this gap (two soft reminders, hard-blocking enforcement gates including the Definition-of-Done pre-commit gate, the opt-in fail-open cross-vendor pre-commit review, one pre-flight context loader, a session-end handoff-readiness detector, and optional safety/observability hooks).
