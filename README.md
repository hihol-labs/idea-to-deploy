...
| `/goal` | **New in v1.44.0.** Long-goal mode — decompose a multi-session goal into ordered verifiable units in `.itd-memory/GOAL.json` (user approval required), drive them one at a time through the standard `/task` pipeline (WIP=1, evidence-gated `verified`), and resume across sessions from the first non-verified unit. An opt-in bounded-autonomy envelope can seal the verifier, cap attempts/time/host-observed tokens, and journal every approach and result; exhaustion stops until the human explicitly raises exactly that budget. Brownfield-first; not a gate — never bypasses `/review`, `/test`, or the DoD. |
