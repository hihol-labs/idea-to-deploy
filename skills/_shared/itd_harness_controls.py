...
CAPABILITY_FIELDS = {"read", "write", "execute"}
DEMAND_GATE_FIELDS = {"status", "evidence", "evidenceSha256"}
SEMANTIC_NAVIGATION_FIELDS = {
    "provider", "languages", "operations", "coverage", "confidence",
    "fallback", "sideEffects", "network", "shell",
}
LANGUAGE_POSTURE_FIELDS = {"python", "typescript"}
SEMANTIC_FALLBACK_FIELDS = {"semantic", "confidence", "mode"}
TRUST_FIELDS = {
...
def validate_tool_trust(registry: dict[str, Any]) -> list[str]:
    ...
    for index, tool in enumerate(tools):
        ...
        seen.add(label)
        for field in ("id", "type", "sideEffects", "externalDataRisk", "fallbackMode"):
            if not nonempty(tool.get(field)):
                issues.append(f"{label}: {field} must be explicit")
        for field in ("usedBy", "approvalRequiredFor"):
            values = tool.get(field)
            if not isinstance(values, list) or not all(nonempty(value) for value in values):
                issues.append(f"{label}: {field} must be an array of strings")
        if not isinstance(tool.get("authNeeded"), bool):
            issues.append(f"{label}: authNeeded must be boolean")
        capabilities = tool.get("capabilities")
        if not isinstance(capabilities, dict) or set(capabilities) != CAPABILITY_FIELDS:
            issues.append(f"{label}: capabilities fields must match the closed schema")
        elif not all(isinstance(value, bool) for value in capabilities.values()):
            issues.append(f"{label}: capabilities values must be boolean")
        demand_gate = tool.get("demandGate")
        if demand_gate is not None:
            if not isinstance(demand_gate, dict) or set(demand_gate) != DEMAND_GATE_FIELDS:
                issues.append(f"{label}: demandGate fields must match the closed schema")
            elif (not all(nonempty(demand_gate.get(field)) for field in ("status", "evidence"))
                  or len(str(demand_gate.get("evidenceSha256") or "")) != 64
                  or any(char not in "0123456789abcdef" for char in str(
                      demand_gate.get("evidenceSha256") or ""))):
                issues.append(f"{label}: demandGate values must match the closed schema")
        semantic = tool.get("semanticNavigation")
        if semantic is not None:
            if not isinstance(semantic, dict) or set(semantic) != SEMANTIC_NAVIGATION_FIELDS:
                issues.append(f"{label}: semanticNavigation fields must match the closed schema")
            else:
                for field in ("provider", "sideEffects"):
                    if not nonempty(semantic.get(field)):
                        issues.append(f"{label}: semanticNavigation.{field} must be explicit")
                for field in ("languages", "operations"):
                    values = semantic.get(field)
                    if not isinstance(values, list) or not all(nonempty(value) for value in values):
                        issues.append(f"{label}: semanticNavigation.{field} must be an array of strings")
                for field in ("network", "shell"):
                    if not isinstance(semantic.get(field), bool):
                        issues.append(f"{label}: semanticNavigation.{field} must be boolean")
                for field in ("coverage", "confidence"):
                    posture = semantic.get(field)
                    if (not isinstance(posture, dict)
                            or set(posture) != LANGUAGE_POSTURE_FIELDS
                            or not all(nonempty(value) for value in posture.values())):
                        issues.append(
                            f"{label}: semanticNavigation.{field} fields must match the closed schema")
                fallback = semantic.get("fallback")
                if (not isinstance(fallback, dict)
                        or set(fallback) != SEMANTIC_FALLBACK_FIELDS
                        or not isinstance(fallback.get("semantic"), bool)
                        or not all(nonempty(fallback.get(field))
                                   for field in ("confidence", "mode"))):
                    issues.append(
                        f"{label}: semanticNavigation.fallback fields must match the closed schema")
        ...
        prompt_bearing = nonempty(trust.get("promptTextSurface"))
        ...
        if prompt_bearing and disposition == "allow" and not reviewed:
        ...
        if prompt_bearing and not reviewed and disposition not in {"ask", "abstain"}:
