...
## 14a. Общее рабочее дерево: стейдж только явным списком (LPD002-A6, 2026-08-20)

В репозитории, где параллельно живёт вторая сессия, `git add -A`/`git add .`
запрещены доктриной: на R5 они затянули чужой WIP (816 строк
`.codex-tmp/ge-plan/build_plan.py`) в дерево, к которому уже были привязаны
машинные квитанции. Ставь файлы ТОЛЬКО перечислением путей (`git add -f` для
tracked-файлов под ignore). Отдельный гейт не строится осознанно: машинная
квитанция привязывает staged-дерево exact-binding'ом — чужой файл в кандидате
валит ревью (что и произошло), второй гейт поверх был бы дублированием.
