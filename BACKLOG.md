...
- [ ] **Класс `ledger-close` не покрывает close-коммит плана.** Критерий R5
...
- [ ] **`git add -A` в общем рабочем дереве затягивает чужой WIP.** На R5
...
- [ ] **`.itd/DECISIONS.md` не трекается git'ом.** `.git/info/exclude:7`
  исключает `.itd/`, и `DECISIONS.md` был заведён уже после этого, поэтому
  журнал durable-решений живёт ТОЛЬКО локально (`git log --all` его не знает),
  тогда как `.itd/ACCEPTANCE_CONTRACT.json` и `.itd/SCOPE_LOCK.md` трекаются —
  они были добавлены раньше исключения. То есть «durable» решения не переживают
  клон репозитория и не видны ревьюеру в дифе. Найдено чекером r4 при разборе
  состава кандидата R5; состояние досталось от R1-R4, не регрессия этого юнита.
  Решать отдельно: либо `git add -f` журналу, либо честно назвать его
  host-local и перестать называть durable.
- [ ] **`_candidate_ledger_facts` вызывает `_staged_file_records` третий раз за
  один `freeze_packet`.** Функция уже дергает его на :1162 и :1184 по разным
