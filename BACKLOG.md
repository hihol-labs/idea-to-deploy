...
Остаток класса:
- [~] **STOPRULE-2** (в работе 2026-09-03, ветка feat/stop-rule-dispositions) — `--emit-dispositions`: при терминале решения владельца
  правило составляет диспозиции ADR-007, человек подписывает одной фразой;
  статус политики `advisory` -> `decides-with-human-confirmation`; закрытие
  PILOT-P02 этим маршрутом (кандидат — `.itd-memory/PILOT-P02-frozen-candidate.patch`).
