...
[![Version: 1.105.0](https://img.shields.io/badge/Version-1.105.0-purple.svg)](CHANGELOG.md)
...
Опциональные [operating loops](docs/OPERATING_LOOPS.md) добавляют бинарную
readiness-диагностику, routing на существующие скиллы, report-only расписания
нативного host’а, временный capability-bound trust, наблюдаемую append-only
телеметрию, детерминированную обработку сбоев и provenance-backed outcome
stories. Отдельный daemon и постоянная unattended authority не появляются.
Закрытый реестр содержит ровно шесть report-first рецептов: state freshness,
test regression, dependency health, security posture, review drift и
documentation drift.
...
~/.claude/plugins/idea-to-deploy/
  ...
  └── hooks/           # хуки-энфорсеры - /adopt и /project предлагают их установить
```
...
**Установка в проект (предлагают `/adopt` и `/project`).** Оба скилла заканчиваются
явным шагом: показывают read-only план
`skills/_shared/itd_project_hooks.py plan --root <проект>` (что добавится в
`<проект>/.claude/settings.json`), спрашивают подтверждение и только после «да»
запускают `itd_project_hooks.py apply --root <проект> --yes`. Слияние идемпотентно
(повторный запуск ничего не добавляет), сохраняет ваши хуки, ключи и правила
`permissions`, пропускает каждый хук, уже зарегистрированный user-level из каталога хуков idea-to-deploy, и никогда не пишет
`~/.claude/settings.json`. При «нет» файл не трогается; та же команда ставит хуки позже.
