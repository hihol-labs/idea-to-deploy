...
> Этот репозиторий — **плагин для Claude Code** (см. `.claude-plugin/plugin.json`). Установка регистрирует 19 скиллов и 6 субагентов в вашем окружении Claude Code — это не самостоятельный CLI.
...
**idea-to-deploy** — это методология, а не просто набор инструментов. 19 скиллов + 6 специализированных агентов, которые превращают Claude Code в профессионального разработчика с проверенным конвейером:
...
2. **Предложить новый скилл** — скиллы живут в `skills/<name>/SKILL.md` и следуют форме существующих 19. Нужны: frontmatter (name, description, triggers, allowed-tools, recommended model), Instructions, Examples, Troubleshooting.
