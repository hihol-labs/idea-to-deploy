...
Методология работает только если Claude реально вызывает скиллы. Совпадения триггеров в `description` необходимы, но недостаточны — под давлением или на неоднозначных промптах Claude может скатиться в ad-hoc tool calls. Папка `hooks/` содержит **24 хука** — но это число смешивает две разные вещи. **8 из них — жёсткие гейты**, которые реально останавливают действие (`permissionDecision: "deny"` на PreToolUse либо `decision: "block"` на SubagentStop, exit 2); остальные **16 — мягкие** (напоминания, инжект контекста, наблюдаемость, самокоррекция — всегда exit 0, никогда не блокируют). Сила enforcement'а харнеса — это **8 жёстких гейтов, а не 24**. Полная таблица (8 жёстких + 16 мягких) и метрика «hard-gate coverage» — в англоязычном [README.md](README.md); тест `tests/verify_gate_taxonomy.py` держит таблицу в синхроне с `hooks/`.
