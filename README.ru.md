...
| `/cross-review` | Обязательная независимая проверка точного staged candidate перед PR. Единый бесплатный маршрут `OpenAI -> Anthropic -> Gemini` запускается через `itd_free_reviewer_producer.py`; только `UNAVAILABLE` переводит к следующему провайдеру, caller bypass отсутствует, а итог принимает Verification Loop. Отдельный платный adapter не является fallback этого маршрута. |
...
| `/cross-review` | Host orchestrator | Другая модель | Оркестрация единого маршрута OpenAI → Anthropic → Gemini без контекста разработки |
