...
**Устаревшие промежуточные записи удалены из дерева PR осознанно.** Ветка
несла ЧЕТЫРЕ live-записи (по одной на каждый reviewed-коммит, менявший
`skills/`); `latest.json` ссылается только на финальную
(`20260820T072138Z-53a19afd`). Три превзойдённые директории
(`…195544Z-8dd74225`, `…203852Z-58aa3061`, `…064228Z-ac470170`) удалены
коммитом ветки: прозрачное представление четырёх транскриптов (~634 KB
декодированных JSONL) превышало иерархический предел ревью-юнитов
(`maxReviewUnits 16 × maxRawDiffBytes 80000`, измеренный отказ
`hierarchical review unit coverage is invalid` на PUB5), а сами записи
остаются в истории ветки (коммиты `3063488`, `58c41b9`, `f5b8755`) и в их
квитанциях. Оракулы после удаления: live-пин 108/0, `verify_tree_pin_debris`
PASSED — ни один tracked-файл на них не ссылается.
