...
существование его критериев). Несовпадение даёт `ROUTE_DEFECT` ДО разбора
...
## 2026-08-24 — LPD-003-3, поправка по двум раундам независимого ревью

**Что изменилось в правиле после ревью.** Первая редакция была fail-open в
четырёх местах; все четыре нашёл независимый cross-vendor ревьюер за два
раунда, и все четыре закрыты корнем.

1. Раунд без сохранённого содержания возвращался ДО проверки исхода: запись с
   `terminal: verdict` и произвольным `outcome` проходила как вердикт. Теперь
   исход судится по словарю СВОЕГО класса терминала.
2. Провенанс пересказа требовал только существования документа. Теперь он
   обязан назвать КОНКРЕТНУЮ строку, и существование строки проверяется
   машинно: «где-то в этом документе» провенансом не является.
3. Механизмы пересказа могли выставить `regression`/`refuted` напрямую, минуя
   требование основания, которое действует для отчётов. Теперь основание
   требуется одинаково: снятие находки со счёта повторов без `why` отвергается.
4. Оракул писал временные файлы с фиксированными именами ВНУТРЬ проверяемого
   дерева. Теперь они живут вне репозитория: оракул не имеет права трогать
   кандидата, которого судит.

**Существенная поправка контракта: повтор требует смены кандидата.** Первая
редакция взводила `REDESIGN_OR_DISCARD` по факту двух вхождений одного ключа,
а «попытка фикса между раундами» была ОБЪЯВЛЕНА следствием устройства маршрута,
а не проверена. Ревьюер показал дыру: два независимых ревью одного и того же
неисправленного кандидата дали бы вердикт «переделать форму». Теперь раунд
несёт личность кандидата (`candidate`), повтор засчитывается только при двух
различимых кандидатах, а запись, которая этого не устанавливает, даёт отдельный
терминал `RECURRENCE_UNCONFIRMED` — правило называет нехватку улики вместо
вердикта.

**Следствие для истории GPG-001.** Её вывод СМЯГЧЁН по уликам, а не по
удобству: журналов промптов по серии `broker-policy` нет (ревью шло
иерархическим API-маршрутом до их введения), поэтому повтор regex-валидации
путей виден, но неотличим от двух взглядов на один кандидат. Терминал —
`RECURRENCE_UNCONFIRMED`. Прежняя запись выше утверждала `REDESIGN_OR_DISCARD`;
это утверждение снимается как не подтверждённое уликами.

**Второй новый терминал: `ROUTE_REPAIR`.** История, в которой вердиктов нет
вовсе или последним сорвался маршрут (`precondition`/`transport`), раньше
проваливалась в `CONTINUE` — то есть звала на новый раунд ревью вместо ремонта
того, что сломалось. Порядок применения теперь: `ROUTE_DEFECT` ->
`REDESIGN_OR_DISCARD` -> `RECURRENCE_UNCONFIRMED` -> `ROUTE_REPAIR` -> `CLOSE`
-> `CONTINUE`.

**Правило сработало на собственном кандидате — записываю как есть.** Прогон по
истории этих двух раундов даёт `REDESIGN_OR_DISCARD` с повтором ключа
`scripts/itd_stop_rule.py::input-validation` (r1, r2). По содержанию это разные
механизмы: исход раунда без содержания против обхода диспозиций в пересказе.
Значит для модуля из одного файла ключ `(файл, категория)` слишком груб — тот
же вывод, что дала грубая группировка на R6, но замеренный на живом юните.
Решение владельца 2026-08-24: находки закрыть и идти в третий раунд; уточнение
зернистости ключа — отдельная работа, записано в BACKLOG.

## 2026-08-24 — LPD-003-3: смена формы проверки провенанса по третьей находке одного класса

**Что.** Проверка провенанса раунда сведена в ОДНУ точку (`validate_provenance`)
выше ветвления по классу терминала. Прежняя форма разводила её по отдельным
ранним выходам для каждой комбинации (терминал x провенанс).

**Почему — по правилу, которое этот же юнит и вводит.** Независимый ревьюер
нашёл один и тот же класс трижды на трёх разных кандидатах: пересказ проверял
существование документа, но не строку (r1); нев-вердиктный раунд мог объявить
себя машинной уликой без пути и sha (r3); он же мог объявить себя пересказом
без документа и строки (публикационный раунд). Ключ механизма
`scripts/itd_stop_rule.py::provenance-validation` повторился при трёх
различимых кандидатах — то есть правило говорит `REDESIGN_OR_DISCARD`, и по
существу право: дефектна форма, а не экземпляр. Четвёртая заплата в том же
классе была бы оплатой того же дефекта ещё раз.

**Как проверено.** Матрица по всем комбинациям класса терминала и класса
провенанса (16 проверок) плюс три обратные мутации единой точки, валящие 2, 8
и 12 проверок. Гарантия теперь одинакова для любой комбинации по построению, а
не по перечислению.

**Цена решения записана честно.** Смена формы обесценила коммит `72b42ca`
(delivery claim на дереве `2757889c` был закрыт чистым PASS пятого раунда) —
потребовалась перечеканка всей цепочки на новом дереве. Это и есть стоимость
позднего распознавания повтора: правило существует ровно для того, чтобы
такое распознавание случалось на втором вхождении, а не на третьем.
